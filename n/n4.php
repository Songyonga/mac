...
<div class="container">
  ...
  <form method="post" action="">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hanziData'])) {
        ...
        for ($i = 0; $i < count($hanzi_list) - 2; $i++) {
            $trigrams[] = $hanzi_list[$i] . $hanzi_list[$i + 1] . $hanzi_list[$i + 2];
        }

        // 3-그램의 빈도 계산
        $trigram_counts = array_count_values($trigrams);
        arsort($trigram_counts); // 빈도수 기준 내림차순 정렬


        // 고유한 3-그램 개수 출력
        $unique_trigram_count = count($trigram_counts);
        echo "<h5 class='text-primary'>논어의 고유 3-그램은 총 $unique_trigram_count 개입니다.</h5>";

        // 상위 10개의 3-그램 출력
        echo "<h5 class='text-primary'>상위 10개의 3-그램 빈도수</h5>";
        echo "<ul>";
        foreach (array_slice($trigram_counts, 0, 10) as $trigram => $count) {
            echo "<li>$trigram: $count 번</li>";
        }
        echo "</ul>";

        // 모든 세 글자 조합 출력 (스크롤 가능한 테이블)
        echo "<div class='result-container'>";
        echo "<table class='table table-bordered'>";
        echo "<thead><tr><th>#</th><th>세 글자</th><th>빈도수</th></tr></thead>";
        echo "<tbody>";

        // 세 글자 조합 출력
        $index = 1;
        foreach ($trigram_counts as $trigram => $count) {
            echo "<tr><td>$index</td><td>" . htmlspecialchars($trigram) . "</td><td>$count</td></tr>";
            $index++;
        }
